...
export function Scene({ isOn, setOn }) {
  return (
    <MotionCanvas orthographic shadows dpr={[1, 2]} camera={{ zoom: 60, position: [-5, 5, 5], fov: 90 }}>
      <motion.group initial={false} animate={isOn ? 'on' : 'off'}>
        <ambientLight intensity={0.1} />
        <directionalLight position={[-20, 20, 20]} intensity={1} />
        <motion.directionalLight position={[-20, -20, -20]} intensity={0.5} variants={colorVariants} />
        <motion.pointLight position={[0, 0, 5]} distance={5} intensity={5} variants={colorVariants} />
        <motion.spotLight
        ...
        <motion.mesh receiveShadow renderOrder={1000} position={[0, 0, 0]} rotation={[-Math.PI / 2, 0, 0]}>
